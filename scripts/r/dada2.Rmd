...
params:
  folder:
    value: /home/mk1b/Projects/nsDNA/pipeline_output/demult.test
  hash:
    value: "yes"
  cont:
    value: "YES"
  fastqs:
    value:  /home/mk1b/Projects/nsDNA/pipeline_output/demultiplexed_20221130_1503/demultiplexed
...
F1s <- sort(list.files(path1, pattern="_FWD.1.fastq.gz", full.names = TRUE))
F2s <- sort(list.files(path1, pattern="_FWD.2.fastq.gz", full.names = TRUE))
R1s <- sort(list.files(path1, pattern="_REV.1.fastq.gz", full.names = TRUE))
R2s <- sort(list.files(path1, pattern="_REV.2.fastq.gz", full.names = TRUE))


tibble(F1s = F1s, 
       F2s = F2s) %>% 
  mutate(sample_name = str_replace(basename(F1s), "_FWD.1.fastq.gz","")) %>% 
...
tibble(R1s = R1s, 
       R2s = R2s) %>% 
  mutate(sample_name = str_replace(basename(R1s), "_REV.1.fastq.gz","")) %>% 
